import sushi from '../public/assets/sushi.png';
import pasta from '../public/assets/pasta.png';
import macaron from '../public/assets/macaron.jpg';
import esfiha from '../public/assets/esfiha.jpg';
import lasanha from '../public/assets/lasanha.jpg';
import tapioca from '../public/assets/tapioca.jpg';
export const ListaProduct = [
    {
        id: 1,
        nome: "Hioki Sushi",
        nota: 4.9,
        descricao: "Peça já o melhor da culinária japonesa no conforto da sua casa! Sushis frescos, sashimis deliciosos e pratos quentes irresistíveis. Entrega rápida, embalagens cuidadosas e qualidade garantida.Experimente o Japão sem sair do lar com nosso delivery!",
        comida: "Japonesa",
        imagem: sushi,
        destaque: "Destaque da semana"
    },
    {
        id: 2,
        nome: "La Dolce Vita Trattoria",
        nota: 4.6,
        descricao: "A La Dolce Vita Trattoria leva a autêntica cozinha italiana até você! Desfrute de massas caseiras, pizzas deliciosas e risotos incríveis, tudo no conforto do seu lar. Entrega rápida, pratos bem embalados e sabor inesquecível. Peça já!",
        comida: "Italiana",
        imagem: pasta
    },
    {
        id: 3,
        nome: "Chatue de la France",
        nota: 3.5,
        descricao: "Delicie-se com os autênticos macarons franceses, pequenas maravilhas crocantes por fora e macias por dentro. Feitos com amêndoas de qualidade, recheios suaves e sabores que vão do clássico ao exótico. Peça já e aproveite essa iguaria francesa sem sair de casa!",
        comida: "Francesa",
        imagem: macaron
    },
    {
        id: 4,
        nome: "Are Baba Restaurante",
        nota: 3.7,
        descricao: "Saboreie nossas deliciosas esfirras, com opções tradicionais de carne, frango, queijo e versões vegetarianas, cada esfirra é preparada com ingredientes frescos e selecionados. Peça já e desfrute do melhor da culinária árabe no conforto do seu lar!",
        comida: "Árabe",
        imagem: esfiha
    },
    {
        id: 5,
        nome: "Restaurante Portuga",
        nota: 4.1,
        descricao: "Delicie-se com nossa lasanha artesanal, preparada com camadas generosas de massa fresca, molhos caseiros e recheios irresistíveis. Seja à bolonhesa, quatro queijos ou vegetariana. Peça já e tenha o melhor da culinária italiana no conforto da sua casa!",
        comida: "Portuguesa",
        imagem: lasanha
    },
    {
        id: 6,
        nome: "Restaurante Brasil",
        nota: 3.5,
        descricao: "Experimente nossa deliciosa tapioca artesanal, feita com massa leve de fécula de mandioca e recheios irresistíveis. Escolha entre opções doces ou salgadas, como coco com leite condensado, queijo derretido ou carne seca. Peça já e saboreie essa iguaria brasileira no conforto da sua casa!",
        comida: "Brasileira",
        imagem: tapioca
    },
]
